...
class UtilityAgent < RubyAgentBase
	...
	def calcCostFromNodeViaLink(link, node, target)
		if getAgentId() then
			...
			# 選択した経路
			if (link.getTags().contains("route1_check")) then
				getAgentTags()[2] = "route1"
			elsif (link.getTags().contains("route2_check")) then
				getAgentTags()[2] = "route2"
			
			# 駅に到着した時刻
			elsif (node.getTags().contains("EXIT_STATION")) then
				time = getCurrentTime().getAbsoluteTime() - 72000

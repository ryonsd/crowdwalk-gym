...
class TwoRoutesEnv(gym.Env):
    def __init__(self, is_gui=False):
        ...
        self.nS = 13
        self.observation_space = gym.spaces.Box(low=0, high=10000, shape=(15,))
    ...
    def reset(self):
        ...
        if self.is_gui:
            subprocess.Popen(["sh", self.path_to_crowdwalk_dir+"quickstart.sh",
                             self.prop_file, "-lError"], stderr=subprocess.DEVNULL)
            ...
            subprocess.Popen(["sh", self.path_to_crowdwalk_dir +
                             "quickstart.sh", self.prop_file, "-c", "-lError"])
